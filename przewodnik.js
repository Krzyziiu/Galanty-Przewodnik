...
const textListPolish = [
    ...
    "Przejdziemy królową polskich ulic, najdłuższym deptakiem (zmierzone!) w Polsce; dotkniemy nosów kilku znanych Łodzian zaklętych w pomniki - pokaże wam też swój mały pomnik; rzucimy okiem na kilka ikonicznych łódzkich kamienic; odwidzimy Aleję Gwiazd polskiej kinematografii; a co jeszcze skrywa ta ulica i jej podwórka dowiecie się w trakcie spaceru.",
...
const pop = (e) =>{
    ...
    if(!popFlag){
        ...
        if(document.title == "Galanta Fremdenführer"){
            popup.children[1].innerText = textListGerman[index];
        }
        else if(document.title == "Galanty Przewodnik"){
            popup.children[1].innerText = textListPolish[index];
        }
        else if(document.title == "Galanta Guide"){
            popup.children[1].innerText = textListEnglish[index];
        }
...
const german = () =>{
    window.location.href = "fremdenführer.html";
}